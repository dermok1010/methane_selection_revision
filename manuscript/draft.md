...
==CH₄ ratio's heritability (0.10 ± 0.01) was the lowest of any definition and was sensitive to the residual-variance structure: under a contemporary-group-mean-class heterogeneous residual — estimable for this trait — it rose to 0.19 (repeatability 0.32), as its near-zero homogeneous-model permanent-environmental variance recovered once the residual was allowed to differ by class, and an independent component-based derivation from the CH₄ and CO₂ (co)variances gave a similar value (~0.19). The estimate reported in Table 2 is the homogeneous-residual value used consistently for all definitions; the sensitivity of this particular trait, and its bearing on the comparison with Jonker et al. [10], is taken up in the Discussion.==
